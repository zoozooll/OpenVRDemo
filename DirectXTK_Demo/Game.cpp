...
void Game::Update(DX::StepTimer const& timer)
{
	...
	m_stars->Update(elapsedTime * 500.0f);
}
...
void Game::Render()
{
	...
	m_spriteBatch->Draw(m_texture.Get(), m_screenPos, NULL, Colors::White, 0.0f, m_origin);
	m_spriteBatch->End();

	m_spriteBatch->Begin();
	//m_spriteBatch->Draw(m_DDSTexture.Get(), DirectX::XMFLOAT2(200.0f, 0.5f), NULL, Colors::White, XM_PIDIV2, m_origin);
	...
	m_d3dContext->OMSetBlendState(m_effectStates->Opaque(), NULL, 0xffffff);
	m_d3dContext->OMSetDepthStencilState(m_effectStates->DepthNone(), 0);
	m_d3dContext->RSSetState(m_effectStates->CullNone());
	m_effect->Apply(m_d3dContext.Get());
	m_d3dContext->IASetInputLayout(m_inputLayout.Get());
	m_primitiveBatch->Begin();
	VertexPositionColor v1({ 400.0f, 150.0f, 0.0f }, Colors::Yellow);
	VertexPositionColor v2({ 600.0f, 450.0f, 0.0f }, Colors::Red);
	VertexPositionColor v3({ 200.f, 450.f, 0.0f }, Colors::Green);
...
void Game::CreateDevice()
{
	...
	DX::ThrowIfFailed(CreateDDSTextureFromFile(m_d3dDevice.Get(), L"cat.dds",
		NULL, m_DDSTexture.ReleaseAndGetAddressOf()));

	DX::ThrowIfFailed(CreateWICTextureFromFile(m_d3dDevice.Get(),
		L"shipanimated.png",
		NULL,
		m_AnimTexture.ReleaseAndGetAddressOf()));
	m_ship = std::make_unique<AnimatedTexture>();
	m_ship->Load(m_AnimTexture.Get(), 4, 20);
	DX::ThrowIfFailed(CreateWICTextureFromFile(m_d3dDevice.Get(), L"starfield.png",
		NULL, m_backgroundTex.ReleaseAndGetAddressOf()));
	...
	m_effectStates = std::make_unique<CommonStates>(m_d3dDevice.Get());
...
void Game::CreateResources()
{
	...
	m_screenPos.x = 0.f;
	m_screenPos.y = (float)(backBufferHeight / 2);
	...
	m_fontPos.x = (float)backBufferWidth / 2.0f;
	m_fontPos.y = (float)backBufferHeight / 2.0f;

	DirectX::SimpleMath::Matrix proj = DirectX::SimpleMath::Matrix::CreateScale(2.0f / float(backBufferWidth),
		-2.0f / float(backBufferHeight),
		1.0f) * 
